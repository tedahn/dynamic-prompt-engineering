...
- **As of:** 2026-07-30
- **Refresh trigger:** Before a current model/capability claim, a model-comparison run, or 30 days after this snapshot.

Checked against official OpenAI sources on 2026-07-30. Model behavior, product controls, and availability are time-sensitive; refresh the sources before making current claims.
...
- Preserve the exact surface label. Do not assume ChatGPT Extra High, Codex Ultra, API `xhigh` or `max`, and API Pro mode are interchangeable controls.
...
For model-tier, reasoning-effort, and capability comparisons, also follow [model-capability-routing-and-evaluation.md](model-capability-routing-and-evaluation.md). Keep prompt effects separate from model, surface, reasoning, tool, and optional-feature effects.
