...
The evaluation candidate applies scope, status, and retrieved-injection filters before relevance-authority-freshness ranking. A routed variant chooses clarification, full context, or composition from declared request and corpus signals. These policies are deliberately inspectable and do not access grader-only expectations.
...
Risks include omitting decisive evidence, compressing away caveats, stale retrieval, adversarial documents, provenance loss, position effects, token-cost overhead, metadata dependence, and overengineering trivial tasks. The current synthetic suite can reward curated retrieval terms and cannot establish answer quality or transfer.

## Evaluation readiness

The local package at `research/evaluations/context-composer/` compares full dump, recency, keyword retrieval, governed composition, and dynamic routing on 12 deterministic fixture families. The 2026-07-29 mechanical snapshot passed its limited pilot gate: both governed conditions had zero restricted/injection, stale, budget, or dependency-ordering failures; baselines retained prohibited or stale items. These are packet-construction observations, not behavioral scores.
