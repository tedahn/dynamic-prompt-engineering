...
- **Documented guidance:** `C-054`, supported by `S-001`, `S-002`, and `S-065` through `S-067`.
- **Design inference:** `C-055`; separating variables should make regressions more attributable, but the local behavioral effect is untested.
- **Primary artifact:** `skills/professionalize-prompt/` and `research/skill-candidates/professionalize-prompt-model-aware.md`.
- **Evaluation:** `E-027`, designed but not run.
