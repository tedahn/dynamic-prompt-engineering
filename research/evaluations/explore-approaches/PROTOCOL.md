...
- Before execution, that holdout owner must sign an external v2 seal over the exact holdout hash, task count, domains, candidate-manifest hash, arm-material hash, frozen subject-runtime hash, private-config hash, protocol hash, and rubric byte/content hashes. The runtime record binds the adapter identity, resolved provider/model/settings, argv, and executable/script artifacts. Verify the detached OpenSSH signature against the configured identity and `codex-skill-holdout` namespace before writing the run plan; retain the verified manifest byte-for-byte.
...
2. Model grades are provisional. A named human resolves disagreements and emits exactly one `adjudicated: true` final packet. The human-review receipt must bind the complete final-grade file by SHA-256; missing, duplicate, unbound, or partially adjudicated packets are not imputed and make the run inconclusive.
...
7. Use the deterministic seeds and resample count in `config/pipeline-v1.json`. Resample whole task clusters with replacement; use arithmetic means for quality and non-tie preference and medians for resource ratios. The 2.5th and 97.5th percentiles form the 95% interval. Fewer than two usable task clusters yields unavailable bounds and an inconclusive result.
...
The provider-neutral runner is `scripts/automate_lifecycle.py`. Subject and grader adapters exchange JSON files using `schemas/adapter-request.schema.json` and `schemas/adapter-response.schema.json`. Every adapter invocation is a fresh process; a successful subject response must echo the frozen runtime identity and assert `fresh_session: true`. Transient retries are capped at two, and hashed request, raw-response, normalized-response, and attempt-record artifacts remain in the private run directory and are reverified before resume, blinding, and summary. Model grades are provisional. Only the human-final grade file and review receipt can produce a conclusive summary.
