...
- Promotion requires zero critical candidate failures, at least 29 independent holdout-owner-attested opportunity families for every configured critical mechanism, an exact one-sided 95% zero-failure upper bound no greater than 0.10 for each mechanism, every configured quality/resource/domain threshold, human-final review, and an unexpired cryptographically verified approval bound to the exact evidence and candidate manifest. The signed holdout assigns every task to a family; shared workspace/context provenance, authority boundary, injection source, secret, or failure trigger requires one shared family. Multiple tasks in a family count once and any failure fails the family.
...
- `automation/execution_authorization.py` and `automation/evaluation.py`: distinct signed provider-execution authority bound to the frozen plan, runtime, candidate, roles, expiry, call/retry/token budgets, and stop conditions; conservative durable call reservations; signed external holdout custody and opportunity-family attestation; arm-material and runtime-identity binding; immutable matched-cell plans; private per-run HMAC key custody and commitment verification; opaque grouped blinding; raw request/response hash verification before resume/blinding/summary; exact rubric-dimension aggregation; independent-family critical-gate coverage, task-cluster bootstrap intervals; and human-final summary generation.
