...
description: Transform vague, underspecified, casual, or low-quality requests into clear, professional, execution-ready prompts optimized for current GPT-5-class and Codex models, including GPT-5.6 Sol, then immediately execute the improved prompt in the same turn by default. Use when a user asks to improve, rewrite, optimize, professionalize, structure, or engineer a prompt; provides a rough request such as "improve this resume"; or would benefit from a stronger internal task specification before execution. Return only the prompt without executing it when the user explicitly requests prompt-only output.
---
...
5. Write an outcome-first prompt suited to the task. Add only sections that change model behavior.
6. Check the prompt against the quality criteria below.
7. Immediately execute the checked prompt in the same turn. Treat it as the working specification, use available tools when appropriate, create requested artifacts, and validate the result in proportion to risk.
8. Return both the professional prompt and the completed result. Do not pause between prompt generation and execution for confirmation unless user input or approval is genuinely required.
...
When the target is GPT-5.6 Sol, the GPT-5.6 family, or current ChatGPT prompting, read [references/gpt-5p6-sol-prompting.md](references/gpt-5p6-sol-prompting.md) before drafting. Treat model behavior, reasoning controls, and product availability as time-sensitive; refresh the linked official sources when current accuracy matters.
